...
pub fn generate_java_jni_bindings(
    ...
    let java_root = convert_to_java_root(&general_root)?;
    ...
    crate::gen_rust::generate_rust_glue(&java_root, rust_out_dir, crate_filter)?;

    Ok(())
}

/// Convert the general IR root to our Java-specific IR root.
fn convert_to_java_root(general_root: &uniffi_bindgen::pipeline::general::Root) -> Result<nodes::Root> {
